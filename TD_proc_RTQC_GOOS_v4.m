%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ... Processing Pati Vela TD data ...
% ...
% ... QC following EuroGOOS for ferrybox data ...
% ... Nina H. Sept-2022
%% ...
...
ifile = sprintf('%s%s','/home/nina/Escritorio/WORK/PatiCientific/data/SPV/SeaWater/SeaDataNet_Format/Profile/RAW/TD/',iyy,'/',imm,'/TD_raw_',td_station,'.txt');
...
A = importdata(ifile);
info=cell2dataset(A.textdata(9:end,1:9));
oCruise = info.Cruise;
Station = info.Station;
Type = info.Type;
TimeOK = info.YYYY_MM_DDThh_mm_ss_sss;
Longitude = info.Longitude_degrees_east_;
Latitude = info.Latitude_degrees_north_;
oLOCAL_CDI_ID = info.LOCAL_CDI_ID;
EDMO_code = info.EDMO_code;
EDMO_code{1}=A.data(1);
BotDepth = info.Bot_Depth_m_;

iTime = cell2mat(A.textdata(10:end,4));
yy = str2num(iTime(:,1:4));
mm = str2num(iTime(:,6:7));
dd = str2num(iTime(:,9:10));
HH = str2num(iTime(:,12:13));
MM = str2num(iTime(:,15:16));
SS = str2num(iTime(:,18:19));
...
for i = 1:length(yy)
    sTimeOK(i,:) = sprintf('%s',sTime(i,1:4),'-',sTime(i,5:6),'-',sTime(i,7:11),':',sTime(i,12:13),':',sTime(i,14:15),'.000');
end

% ... Corrected info to write for SDN file ...
toto = oLOCAL_CDI_ID{1};
tt = sprintf('%s%s%s%s','29PC',toto(1:9),'td',toto(end))
Cruise = oCruise;
Cruise{1} = tt;
LOCAL_CDI_ID = oLOCAL_CDI_ID;
LOCAL_CDI_ID{1} = tt;
...
QC_Depth = A.data(:,4);
Pressure = A.data(:,5);
QC_Pressure = A.data(:,6);
Temperature = A.data(:,7);
QC_Temperature = A.data(:,8);
...
l1=plot(datetime(mTime(1:idm),'convertfrom','datenum'),Temperature(1:idm),'color',colorTempD,'Markersize',10,'Marker','.','LineStyle','none');
hold on
l2=plot(datetime(mTime(idm+1:end),'convertfrom','datenum'),Temperature(idm+1:end),'color',colorTempU,'Markersize',10,'Marker','.','LineStyle','none');
l3=plot(datetime(mTime(idm),'convertfrom','datenum'),Temperature(idm),'Markersize',18,'Marker','*','LineStyle','none','Color',rgb('OrangeRed'));
hleg=legend([l1 l2],'Location','Best');
hleg.String={'Downcast','Upcast'};
title('Temperature [C]');
...
gtitle = sprintf('%s%s','Station ', td_station(10:12));
...
figure(1)
ax1 = subplot(3,4,[1,5,9]) % ... Temperature profile Flags ...
...
clear tt; tt=find(QC_Temperature==4);
if ~isempty(tt)
    l4=plot(datetime(mTime(QC_Temperature==4),'convertfrom','datenum'),double(Temperature(QC_Temperature==4)),'color',colorFlagT4,'Markersize',10,'Marker','.','LineStyle','none');
    hleg=legend([l1 l2 l4],'Location','Best');
    hleg.String={'Downcast','Upcast','RTQC6'};
end
...
ax3 = subplot(3,4,[3,4]) % ... Pressure time serie flags ...
clear tt; tt=find(QC_Pressure==4);
if ~isempty(tt)
    l3=plot(datetime(mTime(QC_Pressure==4),'convertfrom','datenum'),-Pressure(QC_Pressure==4),'color',colorFlagP4,'Markersize',10,'Marker','.','LineStyle','none');
...
clear tt; tt=find(QC_Temperature==3);
if ~isempty(tt)
    l5=plot(datetime(mTime(QC_Temperature==3),'convertfrom','datenum'),double(Temperature(QC_Temperature==3)),'color',colorFlagT4,'Markersize',10,'Marker','.','LineStyle','none');
    hleg=legend([l1 l2 l5],'Location','Best');
    hleg.String={'Downcast','Upcast','RTQC6'};
end
...
clear tt; tt=find(QC_Temperature==4);
if ~isempty(tt)
    l6=plot(Temperature(QC_Temperature==4),-Pressure(QC_Temperature==4),'color',colorFlagT4,'+','LineStyle','none');
    hleg=legend([l1 l2 l6],'Location','Best');
...
clear tt; tt=find(QC_Temperature==4);
if ~isempty(tt)
    l6=plot(datetime(mTime(QC_Temperature==4),'convertfrom','datenum'),double(Temperature(QC_Temperature==4)),'color',colorFlagT4,'Markersize',10,'Marker','.','LineStyle','none');
    hleg=legend([l1 l2 l6],'Location','Best');
    hleg.String={'Downcast','Upcast','RTQC7'};
end
...
clear tt; tt=find(QC_Temperature==4);
if ~isempty(tt)
    l9=plot(datetime(mTime(QC_Temperature==4),'convertfrom','datenum'),double(Temperature(QC_Temperature==4)),'color',colorFlagT4,'Markersize',10,'Marker','.','LineStyle','none');
    hleg=legend([l1 l2 l9],'Location','Best');
    hleg.String={'Downcast','Upcast','RTQC9'};
end
...
clear tt; tt=find(QC_Temperature==4);
if ~isempty(tt)
    l10=plot(datetime(mTime(QC_Temperature==4),'convertfrom','datenum'),double(Temperature(QC_Temperature==4)),'color',colorFlagT4,'Markersize',10,'Marker','.','LineStyle','none');
    hleg=legend([l1 l2 l10],'Location','Best');
    hleg.String={'Downcast','Upcast','RTQC10'};
end
...
clear tt; tt=find(QC_Temperature==3);
if ~isempty(tt)
    l11=plot(datetime(mTime(QC_Temperature==3),'convertfrom','datenum'),double(Temperature(QC_Temperature==3)),'color',colorFlagT3,'Markersize',10,'Marker','.','LineStyle','none');
    hleg=legend([l1 l2 l11],'Location','Best');
    hleg.String={'Downcast','Upcast','RTQC11'};
end
